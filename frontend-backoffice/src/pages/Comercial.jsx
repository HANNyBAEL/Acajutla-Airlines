...
const Comercial = () => {
  ...
  const [fRa, setFRa] = useState({ reservation_id: '', passenger_id: '', flight_id: '', ancillary_id: '' });
  ...
  const cargarReportes = () => {
    ...
    api.get('/comercial/reportes/ancillaries' + q).then((r) => setRepAnc(r.data.datos || [])).catch(() => {});
    api.get('/comercial/reportes/canales' + q).then((r) => setRepCan(r.data.datos || [])).catch(() => {});
  };
  ...
  const notificar = async (id) => {
    try { await api.put('/comercial/waitlist/' + id + '/notificar'); toast.success('Notificado'); cargarBase(); }
    catch (e) { toast.error('Error'); }
  };

  const cancelarWl = async (id) => {
    try { await api.put('/comercial/waitlist/' + id + '/cancelar'); toast.success('Cancelado'); cargarBase(); }
    catch (e) { toast.error('Error'); }
  };
  ...
  const asignarFa = async () => {
    if (!fFa.flight_id || !fFa.ancillary_id || !fFa.stock) return toast.error('Todos los campos son obligatorios');
    try {
  ...
  const addRa = async () => {
    ...
    try {
      ...
      toast.success('Servicio agregado: $' + r.data.datos.precio);
  ...
  const Bar = ({ value, max, color }) => (
    <div className="w-full bg-gray-100 rounded h-4 overflow-hidden">
      <div className={`h-full ${color}`} style={{ width: Math.min(100, (value / (max || 1)) * 100) + '%' }}></div>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">Waitlist, servicios adicionales (ancillaries) y reportes estratégicos</p>
      </div>
      ...
      {tab === 'kpi' && kpi && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="card p-4">
            <div className="text-xs text-gray-500 uppercase">Vuelos hoy</div>
            <div className="text-2xl font-bold mt-1">{kpi.vuelosHoy.programados + kpi.vuelosHoy.en_curso + kpi.vuelosHoy.completados}</div>
            <div className="text-xs mt-2 text-gray-600">
              🟢 {kpi.vuelosHoy.completados} · 🔵 {kpi.vuelosHoy.en_curso} · 🟡 {kpi.vuelosHoy.programados} · 🔴 {kpi.vuelosHoy.cancelados}
            </div>
          </div>
          <div className="card p-4">
            <div className="text-xs text-gray-500 uppercase">Pax últimas 24h</div>
            <div className="text-2xl font-bold mt-1">{kpi.pax24h}</div>
            <div className="text-xs mt-2 text-gray-600">Ocupación hoy: {kpi.ocupacionHoy}%</div>
          </div>
          <div className="card p-4">
            <div className="text-xs text-gray-500 uppercase">Ventas hoy</div>
            <div className="text-2xl font-bold mt-1 text-green-700">${Number(kpi.ventasHoy).toFixed(2)}</div>
            <div className="text-xs mt-2 text-gray-600">Waitlist activas: {kpi.waitlistActivas}</div>
          </div>
          <div className="card p-4">
            <div className="text-xs text-gray-500 uppercase">Salud fiscal hoy</div>
            <div className="text-2xl font-bold mt-1">{kpi.dtesHoy.emitidos}</div>
            <div className="text-xs mt-2">
              {kpi.dtesHoy.rechazados > 0 ? <span className="text-red-600">⚠ {kpi.dtesHoy.rechazados} rechazados</span> : <span className="text-green-600">✓ Sin rechazos</span>}
              {kpi.dtesHoy.contingencia_pendiente > 0 && <span className="text-yellow-600 block">⏳ {kpi.dtesHoy.contingencia_pendiente} en contingencia</span>}
            </div>
          </div>
        </div>
      )}
                  ...
                  waitlist.map((w) => (
                    <tr key={w.id}>
                      ...
                      <td className="px-4 py-2 text-sm">
                        <span className={`px-2 py-1 rounded text-xs ${w.status === 'waiting' ? 'bg-yellow-100 text-yellow-800' : w.status === 'notified' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600'}`}>{w.status}</span>
                      </td>
                      <td className="px-4 py-2 text-sm space-x-2">
                        {w.status === 'waiting' && <>
                          <button className="text-blue-600 hover:underline text-xs" onClick={() => notificar(w.id)}>Notificar</button>
                          <button className="text-red-600 hover:underline text-xs" onClick={() => cancelarWl(w.id)}>Cancelar</button>
                        </>}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="space-y-4">
          <div className="card p-4 grid grid-cols-1 md:grid-cols-5 gap-3">
            ...
            <select className="input-field" value={fAnc.type} onChange={(e) => setFAnc(Object.assign({}, fAnc, { type: e.target.value }))}>
              <option value="baggage">Equipaje</option><option value="seat">Asiento</option>
              <option value="meal">Comida</option><option value="priority">Prioridad</option><option value="other">Otro</option>
            </select>
          ...
          {fFa.flight_id && (
            <div className="card overflow-hidden">
              <div className="px-4 py-2 bg-gray-50 border-b text-sm font-semibold">Servicios disponibles en vuelo seleccionado</div>
              <table className="w-full">
                <tbody className="divide-y">
      ...
      {tab === 'reportes' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="card p-4">
              <h3 className="font-semibold mb-3">Ocupación por ruta</h3>
              {repOcp.length === 0 ? <p className="text-sm text-gray-500">Sin datos</p> :
                <div className="space-y-2">
                  {repOcp.map((r, i) => (
                    <div key={i}>
                      <div className="flex justify-between text-xs mb-1"><span>{r.route_code}</span><span>{r.ocupacion_pct}% ({r.ocupados}/{r.capacidad_total})</span></div>
                      <Bar value={r.ocupacion_pct} max={100} color="bg-primary-500" />
                    </div>
                  ))}
                </div>}
            </div>
            <div className="card p-4">
              <h3 className="font-semibold mb-3">Ingresos por ancillary</h3>
              {repAnc.length === 0 ? <p className="text-sm text-gray-500">Sin datos</p> :
                <div className="space-y-2">
                  {repAnc.map((r, i) => {
                    const max = Math.max(...repAnc.map((x) => x.ingresos));
                    return (
                      <div key={i}>
                        <div className="flex justify-between text-xs mb-1"><span>{r.name}</span><span>${Number(r.ingresos).toFixed(2)} ({r.vendidos})</span></div>
                        <Bar value={r.ingresos} max={max} color="bg-green-500" />
                      </div>
                    );
                  })}
                </div>}
            ...
            <div className="card p-4">
              <h3 className="font-semibold mb-3">Rendimiento por canal</h3>
              {repCan.length === 0 ? <p className="text-sm text-gray-500">Sin datos</p> :
                <table className="w-full text-sm">
                  <tbody className="divide-y">
                    {repCan.map((r, i) => (
                      <tr key={i}>
                        <td className="py-2 capitalize">{r.canal}</td>
                        <td className="py-2">{r.reservas} reservas</td>
                        <td className="py-2 font-semibold">${Number(r.ingresos).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>}
            ...
            <div className="card p-4">
              <h3 className="font-semibold mb-3">Conciliación fiscal (pagos vs DTE)</h3>
              {repConc.length === 0 ? <p className="text-sm text-gray-500">Sin datos</p> :
                <table className="w-full text-sm">
                  <thead><tr className="text-xs text-gray-500 border-b">
                    <th className="py-1 text-left">Fecha</th><th className="text-right">Pagos</th><th className="text-right">DTEs</th><th className="text-right">Dif.</th>
                  </tr></thead>
                  <tbody className="divide-y">
                    {repConc.slice(0, 10).map((r, i) => (
                      <tr key={i}>
                        <td className="py-1">{new Date(r.fecha).toLocaleDateString('es-SV')}</td>
                        <td className="py-1 text-right">${Number(r.total_pagos).toFixed(2)}</td>
                        <td className="py-1 text-right">${Number(r.total_dtes).toFixed(2)}</td>
                        <td className={`py-1 text-right font-semibold ${Math.abs(r.diferencia) > 0.01 ? 'text-red-600' : 'text-green-600'}`}>${Number(r.diferencia).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>}
