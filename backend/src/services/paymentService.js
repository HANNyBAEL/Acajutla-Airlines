...
class PaymentService {
  ...
  static async reembolsar(paymentId, montoReembolso, motivo, usuario) {
    ...
    try {
      ...
      const pago = pagos[0];
      if (montoReembolso > pago.amount) throw new Error('El monto del reembolso no puede ser mayor al pago original');
      ...
      await connection.commit();
      return { exito: true, refund_id: respuesta.refund_id, amount: montoReembolso };
    } catch (error) {
      await connection.rollback();
      throw error;
    } finally {
